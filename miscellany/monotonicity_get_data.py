...
def get_analytic_set(mm, mt, group = 'Disorders'):
    ...
    mm['max_p_comp_'] = mm.groupby(['group', 'corpus', 'mtype', 'sentence'])['p_comp'].transform('max')
    mm['min_p_comp_'] = mm.groupby(['group', 'corpus', 'mtype', 'sentence'])['p_comp'].transform('min')
    mm['max_r_comp_'] = mm.groupby(['group', 'corpus', 'mtype', 'sentence'])['r_comp'].transform('max')
    ...
    if mt == 'precision':
        ...
        test['diff_']=test['max_p_comp_']-test['min_p_comp_']
    elif mt == 'recall':
        test['diff']=test['max_r_comp']-test['min_r_comp']
        test['diff_']=test['max_r_comp_']-test['min_r_comp_']
    elif mt == 'f1':
        test['diff']=test['max_f1_comp']-test['min_f1_comp']
        test['diff_']=test['max_f1_comp_']-test['min_f1_comp_']
        
    test['diff_p']=test['max_p_comp']-test['min_p_comp']
    test['diff_p_']=test['max_p_comp_']-test['min_p_comp_']
    test['diff_r']=test['max_r_comp']-test['min_r_comp']
    test['diff_r_']=test['max_r_comp_']-test['min_r_comp_']

    test['score_diff'] = test['max_score'] - test['min_score']
...
def get_out(mm, group, measures):
    ...
    out['diff_sq'] = out['diff']*out['diff']
    out['diff_sq_sum'] = out.groupby(['corpus', 'sentence', 'mtype', 'group'])['diff_sq'].transform('sum')/2
    out['n_sub_ens']=out.groupby(['corpus', 'mtype', 'group','sentence'])['sentence'].transform('count')/2
    out['diff_mse']=(out['diff_sq_sum'] / out['n_sub_ens']).pow(1./2)
    ...
    out['diff_r_sq'] = out['diff']*out['diff_r']
    ...
    out['max_diff'] = out.groupby(['corpus', 'sentence', 'mtype', 'group'])['diff'].transform('max')
    out['min_diff'] = out.groupby(['corpus', 'sentence', 'mtype', 'group'])['diff'].transform('min')

    #out['add_comp_sq'] = out['add_comp']*out['add_comp']

    out['score_diff']=out.score_diff.astype('float64')
    #out = out.rename(columns={"mtype": "measure", "score_diff": "ma1Gx-min: measure", "diff_": "max-min: complementary measure"})
